const BASIS = (function ()
{
    ...
    return (
        function (BASIS)
        {
            ...
            const Module = typeof BASIS !== 'undefined' ? BASIS : {}; let readyPromiseResolve; let readyPromiseReject;

            Module.ready = new Promise(function (resolve, reject) { readyPromiseResolve = resolve; readyPromiseReject = reject; }); let moduleOverrides = {}; let key;
            ...
            if (Module.noExitRuntime)noExitRuntime = Module.noExitRuntime; if (typeof WebAssembly !== 'object') { err('no native wasm support detected'); } let wasmMemory; const wasmTable = new WebAssembly.Table({ initial: 157, maximum: 157 + 0, element: 'anyfunc' }); let ABORT = false; let EXITSTATUS = 0;
            ...
            function UTF8ArrayToString(heap, idx, maxBytesToRead)
            {
                ...
                while (heap[endPtr] && !(endPtr >= endIdx))++endPtr; if (endPtr - idx > 16 && heap.subarray && UTF8Decoder) { return UTF8Decoder.decode(heap.subarray(idx, endPtr)); }
                ...
                while (idx < endPtr)
                {
                    ...
                    if (!(u0 & 128)) { str += String.fromCharCode(u0); continue; } const u1 = heap[idx++] & 63;

                    if ((u0 & 224) == 192) { str += String.fromCharCode((u0 & 31) << 6 | u1); continue; } const u2 = heap[idx++] & 63;

                    if ((u0 & 240) == 224) { u0 = (u0 & 15) << 12 | u1 << 6 | u2; }
                    else { u0 = (u0 & 7) << 18 | u1 << 12 | u2 << 6 | heap[idx++] & 63; } if (u0 < 65536) { str += String.fromCharCode(u0); }
                    else
                    {
                        const ch = u0 - 65536;
            ...
            } function UTF8ToString(ptr, maxBytesToRead) { return ptr ? UTF8ArrayToString(HEAPU8, ptr, maxBytesToRead) : ''; } function stringToUTF8Array(str, heap, outIdx, maxBytesToWrite)
            {
                ...
                for (let i = 0; i < str.length; ++i)
                {
                    ...
                    } if (u <= 127) { if (outIdx >= endIdx) break; heap[outIdx++] = u; }
                    ...
                    else { if (outIdx + 3 >= endIdx) break; heap[outIdx++] = 240 | u >> 18; heap[outIdx++] = 128 | u >> 12 & 63; heap[outIdx++] = 128 | u >> 6 & 63; heap[outIdx++] = 128 | u & 63; }
                }heap[outIdx] = 0;
            ...
            {
                let endPtr = ptr; let idx = endPtr >> 1; const maxIdx = idx + maxBytesToRead / 2;

                while (!(idx >= maxIdx) && HEAPU16[idx])++idx; endPtr = idx << 1; if (endPtr - ptr > 32 && UTF16Decoder) { return UTF16Decoder.decode(HEAPU8.subarray(ptr, endPtr)); }
                ...
                while (1)
                {
                    ...
                    if (codeUnit == 0 || i == maxBytesToRead / 2) return str; ++i; str += String.fromCharCode(codeUnit);
                }
            ...
            } function lengthBytesUTF16(str) { return str.length * 2; } function UTF32ToString(ptr, maxBytesToRead)
            {
                let i = 0; let str = '';
                ...
                while (!(i >= maxBytesToRead / 4))
                {
                    ...
                    if (utf32 == 0) break; ++i; if (utf32 >= 65536)
                    {
            ...
            function updateGlobalBufferAndViews(buf) { buffer = buf; Module.HEAP8 = HEAP8 = new Int8Array(buf); Module.HEAP16 = HEAP16 = new Int16Array(buf); Module.HEAP32 = HEAP32 = new Int32Array(buf); Module.HEAPU8 = HEAPU8 = new Uint8Array(buf); Module.HEAPU16 = HEAPU16 = new Uint16Array(buf); Module.HEAPU32 = HEAPU32 = new Uint32Array(buf); Module.HEAPF32 = HEAPF32 = new Float32Array(buf); Module.HEAPF64 = HEAPF64 = new Float64Array(buf); } const DYNAMIC_BASE = 5552064; const DYNAMICTOP_PTR = 309024; let INITIAL_INITIAL_MEMORY = Module.INITIAL_MEMORY || 16777216;

            if (Module.wasmMemory) { wasmMemory = Module.wasmMemory; }
            else { wasmMemory = new WebAssembly.Memory({ initial: INITIAL_INITIAL_MEMORY / WASM_PAGE_SIZE, maximum: 2147483648 / WASM_PAGE_SIZE }); } if (wasmMemory) { buffer = wasmMemory.buffer; }INITIAL_INITIAL_MEMORY = buffer.byteLength; updateGlobalBufferAndViews(buffer); HEAP32[DYNAMICTOP_PTR >> 2] = DYNAMIC_BASE; function callRuntimeCallbacks(callbacks)
            {
                while (callbacks.length > 0)
                {
                    const callback = callbacks.shift();

                    if (typeof callback === 'function') { callback(Module); continue; } const func = callback.func;
            ...
            function isDataURI(filename) { return hasPrefix(filename, dataURIPrefix); } const fileURIPrefix = 'file://';

            function isFileURI(filename) { return hasPrefix(filename, fileURIPrefix); } let wasmBinaryFile = 'basis_transcoder.wasm';
            ...
            } function createWasm()
            {
                ...
                return {};
            }__ATINIT__.push({ func() { ___wasm_call_ctors(); } }); const structRegistrations = {};
            ...
            } function simpleReadValueFromPointer(pointer) { return this.fromWireType(HEAPU32[pointer >> 2]); } const awaitingDependencies = {}; const registeredTypes = {}; const typeDependencies = {}; const char_0 = 48; const char_9 = 57;
            ...
            } function __embind_finalize_value_object(structType)
            {
                ...
                delete structRegistrations[structType]; const rawConstructor = reg.rawConstructor; const rawDestructor = reg.rawDestructor; const fieldRecords = reg.fields; const fieldTypes = fieldRecords.map(function (field) { return field.getterReturnType; }).concat(fieldRecords.map(function (field) { return field.setterArgumentType; }));
                ...
                whenDependentTypesAreResolved([structType], fieldTypes, function (fieldTypes)
                {
                    ...
                    {
                        const fieldName = field.fieldName; const getterReturnType = fieldTypes[i]; const getter = field.getter; const getterContext = field.getterContext; const setterArgumentType = fieldTypes[i + fieldRecords.length]; const setter = field.setter; const setterContext = field.setterContext;

                        fields[fieldName] = {
                            read(ptr) { return getterReturnType.fromWireType(getter(getterContext, ptr)); }, write(ptr, o)
                            {
                                const destructors = [];
                    ...
                    return [{
                        name: reg.name, fromWireType(ptr)
                        {
                            const rv = {};

                            for (const i in fields) { rv[i] = fields[i].read(ptr); }rawDestructor(ptr);

                            return rv;
                        }, toWireType(destructors, o)
                        {
                            for (var fieldName in fields) { if (!(fieldName in o)) { throw new TypeError('Missing field'); } } const ptr = rawConstructor();
            ...
            function throwBindingError(message) { throw new BindingError(message); } function registerType(rawType, registeredInstance, options)
            {
            ...
            } function __embind_register_bool(rawType, name, size, trueValue, falseValue)
            {
                ...
                name = readLatin1String(name); registerType(rawType, {
                    name, fromWireType(wt) { return !!wt; }, toWireType(destructors, o) { return o ? trueValue : falseValue; }, argPackAdvance: 8, readValueFromPointer(pointer)
                    {
                        let heap;
            ...
            } function __embind_register_constant(name, type, value)
            {
                ...
                {
                    type = type[0]; Module[name] = type.fromWireType(value);
            ...
            } function __embind_register_enum(rawType, name, size, isSigned)
            {
                ...
                name = readLatin1String(name); function ctor() {}ctor.values = {}; registerType(rawType, { name, constructor: ctor, fromWireType(c) { return this.constructor.values[c]; }, toWireType(destructors, c) { return c.value; }, argPackAdvance: 8, readValueFromPointer: enumReadValueFromPointer(name, shift, isSigned), destructorFunction: null }); exposePublicSymbol(name, ctor);
            ...
            } function __embind_register_enum_value(rawEnumType, name, enumValue)
            {
                ...
                name = readLatin1String(name); const Enum = enumType.constructor; const Value = Object.create(enumType.constructor.prototype, { value: { value: enumValue }, constructor: { value: createNamedFunction(`${enumType.name}_${name}`, function () {}) } });
            ...
            } function floatReadValueFromPointer(name, shift) { switch (shift) { case 2:return function (pointer) { return this.fromWireType(HEAPF32[pointer >> 2]); }; case 3:return function (pointer) { return this.fromWireType(HEAPF64[pointer >> 3]); }; default:throw new TypeError(`Unknown float type: ${name}`); } } function __embind_register_float(rawType, name, size)
            {
                ...
                name = readLatin1String(name); registerType(rawType, {
                    name, fromWireType(value) { return value; }, toWireType(destructors, value)
                    {
            ...
            } function integerReadValueFromPointer(name, shift, signed) { switch (shift) { case 0:return signed ? function readS8FromPointer(pointer) { return HEAP8[pointer]; } : function readU8FromPointer(pointer) { return HEAPU8[pointer]; }; case 1:return signed ? function readS16FromPointer(pointer) { return HEAP16[pointer >> 1]; } : function readU16FromPointer(pointer) { return HEAPU16[pointer >> 1]; }; case 2:return signed ? function readS32FromPointer(pointer) { return HEAP32[pointer >> 2]; } : function readU32FromPointer(pointer) { return HEAPU32[pointer >> 2]; }; default:throw new TypeError(`Unknown integer type: ${name}`); } } function __embind_register_integer(primitiveType, name, size, minRange, maxRange)
            {
                ...
                registerType(primitiveType, {
                    name, fromWireType, toWireType(destructors, value)
                    {
                    ...
                    name, fromWireType(value)
                    {
                        ...
                        if (stdStringIsUTF8)
                        {
                            ...
                            for (var i = 0; i <= length; ++i)
                            {
                                ...
                                {
                                    const maxRead = currentBytePtr - decodeStartPtr; const stringSegment = UTF8ToString(decodeStartPtr, maxRead);
            ...
            } function __embind_register_std_wstring(rawType, charSize, name)
            {
                ...
                else if (charSize === 4) { decodeString = UTF32ToString; encodeString = stringToUTF32; lengthBytesUTF = lengthBytesUTF32; getHeap = function () { return HEAPU32; }; shift = 2; }registerType(rawType, {
                    name, fromWireType(value)
                    {
                        const length = HEAPU32[value >> 2]; const HEAP = getHeap(); let str; let decodeStartPtr = value + 4;
                        ...
                        for (let i = 0; i <= length; ++i)
                        {
                            ...
                            {
                                const maxReadBytes = currentBytePtr - decodeStartPtr; const stringSegment = decodeString(decodeStartPtr, maxReadBytes);
            ...
            } function _abort() { abort(); } function _emscripten_memcpy_big(dest, src, num) { HEAPU8.copyWithin(dest, src, src + num); } function _emscripten_get_heap_size() { return HEAPU8.length; } function emscripten_realloc_buffer(size)
            {
                try
                {
            ...
            {
                requestedSize = requestedSize >>> 0; const oldSize = _emscripten_get_heap_size(); const PAGE_MULTIPLE = 65536; const maxHeapSize = 2147483648;
                ...
                }, join2(l, r) { return PATH.normalize(`${l}/${r}`); }
            }; var SYSCALLS = {
                mappings: {}, buffers: [null, [], []], printChar(stream, curr)
                {
            ...
            {
                args = args || arguments_; if (runDependencies > 0) { return; }preRun(); if (runDependencies > 0) return; function doRun() { if (calledRun) return; calledRun = true; Module.calledRun = true; if (ABORT) return; initRuntime(); preMain(); readyPromiseResolve(Module); if (Module.onRuntimeInitialized)Module.onRuntimeInitialized(); postRun(); } if (Module.setStatus) { Module.setStatus('Running...'); setTimeout(function () { setTimeout(function () { Module.setStatus(''); }, 1); doRun(); }, 1); }
                else { doRun(); }
